...
import '../styles/TwitterComment.css'; 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://localhost:8000/api/scrape_comments/',
                { url },
                {
                    headers: {
            ...
            const analyzedComments = await Promise.all(fetchedComments.map(async (comment) => {
                ...
                return {
                    ...
                    sentimentScore: sentimentResponse.data.score.toFixed(2),
                    emotion: emotionResponse.data.emotion,
                    emotionProbability: emotionResponse.data.probability.toFixed(2)
                };
            }));
            ...
            await axios.post(
                'http://localhost:8000/api/twitter-comment-history/',
                {
        ...
        <div className='container'>
            ...
            <form onSubmit={handleSubmit}>
                <input
                    ...
                    required
                />
                <button type="submit" className="twittercomment-button" disabled={loading}>
                    {loading ? 'Loading...' : 'Get Comments'}
                </button>
                <button type="button" className="twittercomment-reset-button" onClick={handleReset}>Reset</button>
                <button type="button" className="twittercomment-visualization-button" onClick={handleToggleVisualization}>
                    {showVisualization ? 'Back' : 'Get Visualization'}
                </button>
            </form>
            {error && <p style={{ color: 'red' }}>{error}</p>}
            <div className={`tweet-comments-container ${showVisualization ? 'slide-left' : 'slide-right'}`}>
            ...
            <div className={`twitter-comments-container ${showVisualization ? 'slide-left' : 'slide-right'}`}>
                ...
                {loading ? (
                    ...
                    <ul>
                        {comments
                            ...
                            .map((comment, index) => (
                                <li key={index}>
                                    <p><strong>{comment.username}</strong> ({comment.date}): {comment.text}</p>
